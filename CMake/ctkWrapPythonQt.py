...
""").substitute(namespace = namespace, target = target, registerclasses = '\n'.join(registerclasses)))
    ...
    output_h = output_dir + "/" + namespace + "_" + target + "_init.h"
    if extra_verbose:
        print("output_h: %s" % output_cpp)
...
#ifndef __${namespace}_${target}_init_h
#define __${namespace}_${target}_init_h
...
#endif
""").substitute(namespace = namespace, target = target))
